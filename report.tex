...
The four-momentum components were stored as private double member variables within the FourMomentum class, as this is a more readable format than a vector in which the index corresponding to each property is not explicit to anyone extending the class.
...
\begin{figure} 
	...
	\label{particleuml}
\end{figure}
To create classes for the different types of particles, an inheritance hierarchy was used, shown in Figure \ref{particleuml}. For inheritance, the destructors of the parent class must be defined and virtual, and destructors defined in child class. This is so the child and parent class destructor is called when the object goes out of scope, instead of just the parent class destructor. 	
...
\begin{lstlisting}[firstnumber=108, caption={Code snippet from ParticleBunch.cpp showing the use of polymorphism in deciding which particles to decay and their results.}, label=sbs]
...
\begin{lstlisting}[firstnumber=13, caption={Code snippet from ParticleType.cpp showing the string\_to\_particle\_type conversion function.}, label=strtopt]
