...
To simulate these, object oriented programming (OOP) was used. OOP has four main concepts: encapsulation, inheritance, polymorphism and abstraction.
Encapsulation is to make properties of an object private, and only accessible through public member functions of the class. This reduced access can allow for input validation or user friendly interfaces and reduce unnecessary user access to data. 
Inheritance aims to increase the reusability of code by allowing child classes to gain the functions and properties of their parent class, with a side effect of reducing variance in interface for similar classes. 
Polymorphism is reusing function names to  
...
Particle also contains friend functions in the form of sum and dot\_product, which take another particle as input, Listing \ref{friend}, outputting FourMomentum and doubles respectively. They use the addition operator overload and dot\_product function mentioned in Section \ref{Foir}
\begin{lstlisting}[firstnumber=51, caption={Code snippet from Particle.cpp showing the friend functions sum and dot\_product in Particle class.}, label=friend]
// Sum friend function
FourMomentum sum(const Particle &particle1, const Particle &particle2)
{
  return *particle1.four_momentum_ptr_ + *particle2.four_momentum_ptr_;
}
// Dot product friend function
double dot_product(const Particle &particle1, const Particle &particle2)
{
  return particle1.four_momentum_ptr_
	->dot_product(*particle2.four_momentum_ptr_);
}
...
ParticleType 
 file also
contains conversion functions for string to ParticleType and ParticleType to string, abstracting the validation of particle type inputs into one place. The 
string\_to\_particle\_type function, Listing \ref{strtopt} , uses an unordered map to store key-value pairs, so the string can be looked up and the equivalent ParticleType returned. Using a map allows for multiple strings to correspond to the same ParticleType, such as with "positron" and "anti\_electron" both converting to ParticleType::POSITRON.
...
std::string Detector::predict_particle(
	double emcal_energy, double hcal_energy,
